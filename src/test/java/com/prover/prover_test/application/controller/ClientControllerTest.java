...
class ClientControllerTest {
    ...
    void setUp(@Autowired ClientController clientController) {
        org.springframework.test.util.ReflectionTestUtils.setField(clientController, "service", clientService);
    }

    @Test
    void shouldReturnAllClients() throws Exception {
        var client = new ClientResponse(UUID.randomUUID(), "Fulano", "fulano@mail.com", "999999999", null);
    ...
    void shouldSearchClientsByName() throws Exception {
        ...
        mockMvc.perform(get("/api/clients/search").param("name", "Bel"))
                .andExpect(status().isOk())
    ...
    void shouldImportClientsFromCsv() throws Exception {
        ...
        mockMvc.perform(multipart("/api/clients/import").file(file))
                .andExpect(status().isOk());
    }
}
